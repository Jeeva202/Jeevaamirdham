...
export default function Dashboard({ showAlert }) {
    ...
    const { data: planData, status: planStatus, isLoading: planIsLoading, error: planError } = useQuery({
        queryFn: async () => {
            const { data } = await axios.get(process.env.REACT_APP_URL + `/getPlanEvenItisExpired`, {
                params: {
                    ...
                    {planData ? (
                        <>
                            {isExpired ? (
                                ...
                                <Alert severity="info">
                                    <h3>
                                        Your {planData?.name?.split(" ")
                                            .map(word => word.charAt(0).toUpperCase() + word.slice(1).toLowerCase())
                                            .join(' ')} Plan has been Expired
                                    </h3>
                                    Your Magazine Subscription starts from <strong>{expiryData?.created_dt?.slice(0, 10)}</strong> Ends on <strong>{expiryData?.expiry_dt?.slice(0, 10)}</strong>.
