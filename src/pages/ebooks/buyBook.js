...
import CartModal from "../../components/cart/cartModal";
import {
    Pagination, Badge, Card, CardContent, Button, Typography, IconButton
} from "@mui/material";
...
export default function BuyBook({
    ...
    // Fetch favorite books on mount
    const fetchFavorites = async () => {
        try {
            const response = await axios.get(`${process.env.REACT_APP_URL}/favorites`, {
                params: { userId }
            });
    ...
    const addToFavorites = async (bookId) => {
        try {
            const response = await axios.post(`${process.env.REACT_APP_URL}/addFavorites`, {
                userId,
                bookId
            });
            setFavorites(response.data.favorites);
    ...
    const removeFromFavorites = async (bookId) => {
        try {
            const response = await axios.post(`${process.env.REACT_APP_URL}/deleteFavorites`, {
                userId,
                bookId
            });
            setFavorites(response.data.favorites);
    ...
    const toggleFavorite = async (bookId, event) => {
        event.stopPropagation();
    ...
    return (
        <div className="shop-section" style={{ padding: '1rem' }}>
            ...
            <div className="shopcontent" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                <div className="buy-section" style={{ display: 'flex', flexWrap: 'wrap', gap: '16px', justifyContent: 'center' }}>
                    {currentItems.map((e, i) => (
                        <Card className="book-card" key={i} variant="outlined" onClick={() => navigateToProductSpecificPage(i)} style={{
                            ...
                            <CardContent className="content-section" style={{ padding: '1rem', width: '100%', paddingBottom: '0', flex: 'none' }}>
                                ...
                                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '0.5rem' }}>
                                    ...
                                    <IconButton
                                        onClick={(event) => toggleFavorite(e.id, event)}
                                        sx={{
                                            border: '2px solid #F09300 !important',
                                            borderRadius: '50%',
                                            padding: '6px !important',
                                            background:'none !important',
                                        }}
                                    >
                                        {favorites.includes(e.id) ? <FavoriteIcon sx={{ color: '#F09300' }} /> : <FavoriteBorderIcon sx={{ color: '#F09300' }} />}
                                    </IconButton>
                                </div>
                            </CardContent>
                        </Card>
